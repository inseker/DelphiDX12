...
begin
    ...
    begin
        pDestSlice := pDest^.pData + pDest.SlicePitch * z;
        pSrcSlice := pSrc.pData + pSrc.SlicePitch * z;
        for y := 0 to NumRows - 1 do
        begin
            p1 := pSrcSlice + pSrc.RowPitch * y;
            p2 := pDestSlice + pDest^.RowPitch * y;
            Windows.CopyMemory(p2, p1, RowSizeInBytes);
            //  move(p1, p2,RowSizeInBytes );  // FastMove makes here something wrong...
        end;
    end;
end;
...
begin
    ...
    pLayouts := pMem;
    pRowSizesInBytes := pointer(pLayouts) + SizeOf(TD3D12_PLACED_SUBRESOURCE_FOOTPRINT) * NumSubresources;
    pNumRows := pointer(pRowSizesInBytes) + Sizeof(UINT64) * NumSubresources;
...
var
    ...
    pParameters: pointer;
    pParameters_1_0: array of TD3D12_ROOT_PARAMETER;
    n: UINT;
    table_1_1: TD3D12_ROOT_DESCRIPTOR_TABLE1;
    DescriptorRangesSize: SIZE_T;
    pDescriptorRanges: pointer;
    pDescriptorRanges_1_0: array of TD3D12_DESCRIPTOR_RANGE;
    x: uint;
                        ...
                        begin
                            ...
                            case (desc_1_1.GetParameter(n).ParameterType) of
                                ...
                                begin
                                    ...
                                    pDescriptorRanges_1_0 := (pDescriptorRanges);
...
var
    ...
    pStream: PBYTE;
    SubobjectSeen: array [0..D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_MAX_VALID - 1] of longbool;
...
begin
    ...
    begin
        ...
        begin
           ...
           SizeOfSubobject := sizeof(TD3DX12_PIPELINE_STATE_STREAM.DS);
           end;
        D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_HS:
        begin
           pCallbacks.HSCb( PD3D12_SHADER_BYTECODE(pStream));
           SizeOfSubobject := sizeof(TD3DX12_PIPELINE_STATE_STREAM.HS);
           end;
        D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_GS:
        begin
           pCallbacks.GSCb (PD3D12_SHADER_BYTECODE(pStream));
           SizeOfSubobject := sizeof(TD3DX12_PIPELINE_STATE_STREAM.GS);
           end;
        D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_CS:
        begin
           pCallbacks.CSCb( PD3D12_SHADER_BYTECODE(pStream));
           SizeOfSubobject := sizeof(TD3DX12_PIPELINE_STATE_STREAM.CS);
           end;
